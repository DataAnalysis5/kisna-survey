    ...
    responses.forEach((response) => {
      if (response.surveyId?.questions) {
        response.surveyId.questions.forEach((question, index) => {
          questionHeaders.add(`Q${index + 1}: ${question.text}`)
        })
      }
    })

    // Convert to array and sort
    const sortedHeaders = Array.from(questionHeaders).sort()

    // Second pass: format data with questions as columns
    responses.forEach((response) => {
      const baseData = {
        "Employee Code": response.employeeCode || "NA",
        Name: response.name || "NA",
        Department: response.department || "Unknown Department",
        Designation: response.designation || "Unknown Designation",
      ...
      // Add question answers as columns
      if (response.surveyId?.questions) {
        response.surveyId.questions.forEach((question, index) => {
          const questionHeader = `Q${index + 1}: ${question.text}`
          const answer = response.answers[`q${index}`] || "No answer"
          baseData[questionHeader] = answer
        })
      }
...
app.post("/api/responses", async (req, res) => {
  try {
    ...
    const { surveyId, userId, department, designation, answers, name, employeeCode } = req.body

    // Create response object
    const response = new Response({
      surveyId,
      userId: userId || "anonymous_user", // Keep userId for tracking
      employeeCode: employeeCode || "NA", // Store employee code
      name: name || "NA",
      location: "Not specified", // Set default location since it's no longer collected
      department,
      answers,
      designation, // Store designation instead of tenure
    })
